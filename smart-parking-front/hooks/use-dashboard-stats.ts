...
export function useDashboardStats() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const [summaryResponse, operatorsResponse, passagesResponse, completedPassagesResponse] = await Promise.all([
        // Dashboard summary (passages, revenue)
        get<ApiResponse<DashboardSummary>>(API_ENDPOINTS.VEHICLE_PASSAGES.DASHBOARD_SUMMARY),
        // Operators list
        get<ApiResponse<any> | PaginatedApiResponse<any>>(API_ENDPOINTS.OPERATORS.LIST),
        // Recent passages (for activity and charts)
        get<ApiResponse<any> | PaginatedApiResponse<any>>(`${API_ENDPOINTS.VEHICLE_PASSAGES.LIST}?per_page=100`),
        // Completed passages for revenue calculations
        get<ApiResponse<any> | PaginatedApiResponse<any>>(`${API_ENDPOINTS.VEHICLE_PASSAGES.COMPLETED_LIST}?per_page=200`),
      ]);
...
function calculateWeeklyRevenue(passages: VehiclePassage[]): RevenueDataPoint[] {
  ...
  passages.forEach(passage => {
    ...
    if (diffDays < 7 && diffDays >= 0) {
      ...
      if (weekData[dayName]) {
        weekData[dayName].revenue += passage.total_amount || 0;
        weekData[dayName].vehicles += 1;
      }
    }
  });
  
  // Convert to array maintaining order (last 7 days)
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    result.push({
      name: dayName,
      revenue: weekData[dayName]?.revenue || 0,
      vehicles: weekData[dayName]?.vehicles || 0,
    });
  }
...
function calculateHourlyTraffic(passages: VehiclePassage[]): HourlyDataPoint[] {
  ...
  // Return only business hours (6AM - 10PM) for cleaner chart
...
function calculateVehicleTypeDistribution(passages: VehiclePassage[]): VehicleTypeDataPoint[] {
  ...
  if (total === 0) {
    // Return default data if no passages
    return [
      { name: 'Cars', value: 65, color: VEHICLE_TYPE_COLORS['Car'] },
      { name: 'Motorcycles', value: 25, color: VEHICLE_TYPE_COLORS['Motorcycle'] },
      { name: 'Trucks', value: 10, color: VEHICLE_TYPE_COLORS['Truck'] },
    ];
  }
