...
export function CameraExitDialog({
  ...
  const handleProcessExit = async (bodyTypeId?: number) => {
    ...
    const vehicle = detection.vehicle || detection.active_passage?.vehicle;
    
    // Check if vehicle has body_type_id, if not, show selection modal
    if (!vehicle?.body_type_id && !bodyTypeId) {
    ...
    try {
      const result = await CameraDetectionService.processExitDetection(
        detection.id,
        {
          ...
          body_type_id: bodyTypeId || vehicle?.body_type_id,
        }
      );
      ...
      if (!result.success && (result.data?.result as any)?.gate_action === 'require_vehicle_type') {
        setShowVehicleTypeModal(true);
      ...
      if (result.success) {
        ...
        const passage = result.data?.passage;
        const exitResult = result.data?.result as any;
        if (exitResult?.receipts && exitResult.receipts.length > 0 && !pricingPreview?.no_fee && !isPaidPass) {
          try {
            const { generateReceiptHTML, printContent } = await import("@/utils/print-utils");
            const receipt = exitResult.receipts[0];
            const vehicle = detection?.vehicle || detection?.active_passage?.vehicle;
            const receiptData = {
              receiptId: receipt.receipt_number || receipt.id,
              passageNumber: passage?.passage_number || detection?.active_passage?.passage_number,
              plateNumber: vehicle?.plate_number || detection?.numberplate || "",
              vehicleType: vehicle?.body_type?.name || "Unknown",
              entryTime: formatDateTime(passage?.entry_time || detection?.active_passage?.entry_time),
              exitTime: formatDateTime(passage?.exit_time),
              gate: selectedGate?.name || "",
              passageType: passage?.passage_type || "toll",
              paymentMethod: receipt.payment_method || "Cash",
              amount: receipt.amount || passage?.total_amount || 0,
            };
  ...
  const handleVehicleTypeSelected = async (bodyTypeId: number) => {
    ...
    // Get pricing preview before processing exit
    ...
    try {
      ...
      const preview = await VehiclePassageService.getExitPricingPreview({
        plate_number: vehicle?.plate_number || detection?.numberplate || "",
      ...
      if (preview.success && preview.data) {
        ...
        if (preview.data.paid_pass_active) {
          console.log('Paid pass is active for vehicle:', vehicle?.plate_number || detection?.numberplate);
        }
      } else {
  ...
  const handleConfirmExit = async () => {
    ...
    await handleProcessExit(selectedBodyTypeId || undefined);
  };
  ...
  const needsVehicleType = !vehicle?.body_type_id;
  
  // Check for paid pass status from vehicle (paid_until), pricing preview, or exit result
  const paidUntil = vehicle?.paid_until ? new Date(vehicle.paid_until) : null;
  const isPaidFromVehicle = !!paidUntil && paidUntil.getTime() > Date.now();
  const isPaidPass = isPaidFromVehicle || pricingPreview?.paid_pass_active === true || (exitResult?.data as any)?.paid_pass_active === true;

  // Calculate duration and fee (daily - rolling 24-hour periods)
  const entryTime = new Date(activePassage.entry_time);
  const now = new Date();
  const durationMs = now.getTime() - entryTime.getTime();
  const durationHours = durationMs / (1000 * 60 * 60);
  const durationDays = Math.ceil(durationHours / 24); // Round up to next full day
  const durationHoursRemainder = Math.floor(durationHours % 24);
  const durationMinutes = Math.floor((durationMs % (1000 * 60 * 60)) / (1000 * 60));
  const durationText = durationDays > 1 
    ? `${durationDays} days, ${durationHoursRemainder}h ${durationMinutes}m`
    : `${Math.floor(durationHours)}h ${durationMinutes}m`;
  
  // Calculate fee (base_amount * days) - base_amount now represents daily rate
  const baseAmount = activePassage.base_amount || 0;
  const totalAmount = baseAmount * durationDays;
